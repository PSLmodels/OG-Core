'''
OG-USA abstract base parameters class.
'''
# CODING-STYLE CHECKS:
# pep8 parameters.py
...
class ParametersBase(object):
    ...
    def default_data(cls, metadata=False):
        ...
        Args:
        metadata (bool): whether to return parameter metadata

        Returns:
        params (dict): dictionary of data
    ...
    def _params_dict_from_json_file(cls):
        ...
        Returns:
        params (dict): dictionary containing complete contents of
            DEFAULTS_FILENAME file.
    ...
    def _update(self, mods):
        ...
        Args:
            mods (dict): dictionary containing a parameter:value pairs

        Returns:
            None

        Raises:
            ValueError: if mods is not a dict.
    ...
    def _expand_array(x, x_int, x_bool, x_string):
        ...
        Args:
            x (object): value to expand
                x must be either a scalar list or a 1D numpy array, or
                x must be either a list of scalar lists or a 2D numpy
                array
            x_int (bool):
            ...
            x_string (bool):
                True implies x has dtype=np.str_;
                False implies x has dtype=np.float64 or dtype=np.int8

        Returns:
            x (Numpy array): expanded numpy array with specified dtype
    ...
    def simple_eval(self, param_string):
        ...
        Parses `param_string` and returns result. `param_string can be
            ...
            1. `param_name op scalar` -- this will be parsed into param,
                op, and scalar where `op` is a key in `OP_DICT`. The
                corresponding function is applied to the parameter value
                and the scalar value.
            2. `param_name` -- simply return the parameter value that is
                retrieved from the object

        Args:
            param_string (str): of form `param op scalar` or `param`

...
def test_euler_equation_solver():
    ...
    p.tax_func_type = 'DEP'
    p.lambdas = lambdas.reshape(p.J, 1)
    b_splus1 = np.array(guesses[:p.S]).reshape(p.S, 1)
    ...
    print('Test array = ', test_list)
    print('Expected array = ', expected_list)
    print('Max abs diff = ', np.absolute(np.array(test_list) - np.array(expected_list)).max())
    print('j = ', j)

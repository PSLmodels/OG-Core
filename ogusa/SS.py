...
def euler_equation_solver(guesses, params):
    ...
    tax_params = length 4 tuple, (analytical_mtrs, etr_params,
                 mtrx_params, mtry_params)
    analytical_mtrs = boolean, =True if use analytical_mtrs, =False if
                       use estimated MTRs
    etr_params      = [S,BW,#tax params] array, parameters for effective
...
def inner_loop(outer_loop_vars, params, baseline, baseline_spending=False):
    ...
    if budget_balance:
        T_H_params = (e, lambdas.reshape(1, J), omega_SS.reshape(S, 1),
                      'SS', etr_params, theta, tau_bq, tau_payroll,
...
def SS_solver(b_guess_init, n_guess_init, rss, wss, T_Hss, factor_ss,
              Yss, params, baseline, fsolve_flag=False,
...
#        raise RuntimeError(err)
    ...
    lump_sum_params = (e, lambdas.reshape(1, J), omega_SS.reshape(S, 1),
                       ...
                       h_wealth, p_wealth, m_wealth, retire, T, S, J,
                       tau_b, delta_tau)
    revenue_ss = aggr.revenue(new_r, new_w, b_s, nssmat, new_BQ, Yss,
    ...
    mtry_ss_params = (e_extended[1:, :], etr_params_extended_3D,
                      mtry_params_extended_3D, analytical_mtrs)
...
def run_SS(income_tax_params, ss_params, iterative_params, chi_params,
    ...
    INPUTS:
    income_tax_parameters = length 4 tuple, (analytical_mtrs, etr_params,
                            mtrx_params, mtry_params)
    ...
    if baseline:
        ...
        [rguess, wguess, T_Hguess, Yguess, factor] =\
        ...
        solution_params = [b_guess.reshape(S, J), n_guess.reshape(S, J),
                           ...
                           iterative_params, small_open_params]
        output = SS_solver(b_guess.reshape(S, J), n_guess.reshape(S, J),
                           rss, wss, T_Hss, factor, Yss, solution_params,

...
def tax_rates(params, X, Y, tax_func_type, rate_type):
    '''
    Generates tax rates given income data and the parameters of the tax
    functions.  Used in parameter_plots.plot_2D_taxfunc().
    ...
    if tax_func_type == 'GS':
        ...
        (A, B, _, _, max_income, min_income, _, _, shift_income, _,
         shift) = params
        income2 = income ** 2
    ...
    else:  # case for DEP or linear tax functions
        A, B, C, D, max_x, min_x, max_y, min_y, shift_x, shift_y, shift, share = params

...
\section{Model with Endogenous Labor}\label{SecModel}
  ...
  \subsection{Individual problem}\label{SecIndProb}
    ...
    \begin{equation}\label{EqSmaxprob}
      \begin{split}
        \max_{c_{j,S,t},n_{j,S,t},b_{j,s+1,t+1}} &\frac{\left(c_{j,S,t}\right)^{1-\sigma} - 1}{1 - \sigma} + \chi\frac{(\tilde{l}-n_{j,s,t})^\eta}{\eta} \\
    ...
    \begin{gather}
      ...
      w_{t+1} e_j(S)\Bigl[\left(1+r_{t+1}\right)b_{j,S,t+1} + w_{t+1} e_j(S)n_{j,S,t+1}\Bigr]^{-\sigma} = \chi(\tilde{l} - n_{j,S,t+1})^{\eta-1} \quad\forall t \label{EqEulerSlab2} \\
      \begin{split}
    ...
    \begin{gather}
      \begin{split}
        w_t e_j(s)\Bigl[\left(1+r_t\right)b_{j,s,t} + &w_t e_j(s)n_{j,s,t} - b_{j,s+1,t+1}\Bigr]^{-\sigma} = \chi(\tilde{l} - n_{j,s,t})^{\eta-1} \\
  ...
  \subsection{Market clearing and equilibrium}\label{SecMCandEqlbm}
    ...
    \begin{gather}
      \begin{split}
        \bar{w}e_j(s)\Bigl[\left(1+\bar{r}\right)\bar{b}_{j,s} + &\bar{w} e_j(s)\bar{n}_{j,s} - \bar{b}_{j,s+1}\Bigr]^{-\sigma} = \chi(\tilde{l} - \bar{n}_{j,s})^{\eta-1} \\
    ...
    \begin{figure}[htb]\centering \captionsetup{width=4.0in}
      ...
      \fbox{\resizebox{4.0in}{3.0in}{\includegraphics{images/SavSS.pdf}}}
    \end{figure}

    \begin{figure}[htb]\centering \captionsetup{width=4.0in}
      \caption{\label{FigLabSS}\textbf{Steady-state distribution of individual labor supply $\bar{n}_{j,s}$ for $S=60$ and $J=7$}}
      \fbox{\resizebox{4.0in}{3.0in}{\includegraphics{images/LabSS.pdf}}}
    \end{figure}

    Figure \ref{FigSavSS} shows the steady-state distribution of individual savings $\bar{\bm{\Gamma}}$ and Figure \ref{FigLabSS} shows the steady-state distribution of individual labor supply $\bar{n}_{j,s}$ for a particular calibration of the model. The deterministic individual ability process $e_j(s)$ is calibrated from CPS wage distribution data as described in Appendix \ref{AppAbilCalib}, with $S=60$ and $J=7$. We calibrate the other parameters to $[\beta,\sigma,\delta,\chi,\eta,\alpha,A] = [0.96,3,0.05,1,1.5,0.35,1]$. Notice .... %that some intergenerational risk sharing is taking place. Young individuals of high ability, know that they will have high ability and borrow $b_{j,s,t}<0$ in their early years in the steady-state.
    ...
    \begin{figure}[htb]\centering \captionsetup{width=4.0in}
      ...
      \fbox{\resizebox{4.0in}{3.0in}{\includegraphics{images/KpathTPI.pdf}}}
    \end{figure}

...
\section{Model with Endogenous Labor}\label{SecModel}
  ...
  \subsection{Market clearing and stationary equilibrium}\label{SecMCEqlbm}
    ...
    Figure \ref{FigSavSS} shows the stationary steady-state distribution of individual savings $\bar{\bm{\Gamma}}$ and Figure \ref{FigLabSS} shows the stationary steady-state distribution of individual labor supply $\bar{n}_{j,s}$ for a particular calibration of the model. The deterministic individual ability process $e_{j,s}$ is calibrated from CPS wage distribution data as described in Appendix \ref{AppAbilCalib}, with $S=60$ and $J=7$. We calibrate the other parameters to $[\beta,\sigma,\delta,\chi,\eta,\alpha,A] = [0.96,3,0.05,1,2.5,0.35,1]$. Notice .... %that some intergenerational risk sharing is taking place. Young individuals of high ability, know that they will have high ability and borrow $b_{j,s,t}<0$ in their early years in the steady-state.
...
\section{Constraints on individual borrowing}\label{AppBorConstr}
  ...
  \begin{equation}\label{EqBorConsts}
    \begin{split}
      \tilde{b}_{j,S,t} &= \frac{\tilde{c} - w_te_{j,S}\tilde{l}}{1+r_t}  \\
      \tilde{b}_{j,S-1,t-1} &= \frac{\tilde{c} + \tilde{b}_{j,S,t} - w_{t-1}e_{j,S-1}\tilde{l}}{1+r_{t-1}} \\
      &\vdots \\
      \tilde{b}_{j,2,t-S+2} &= \frac{\tilde{c} + \tilde{b}_{j,3,t-S+3} - w_{t-S+2}e_{j,2}\tilde{l}}{1+r_{t-S+2}}
    \end{split}
  \end{equation}
...
\section{Solving for stationary non-steady-state equilibrium by time path iteration}\label{AppNonSSsolve}
  ...
  \end{equation}
  In Section \ref{SecMCEqlbm}, we describe how the stationary non-steady-state equilibrium time path of allocations and price is described by functions of the state $\bm{\hat{\Gamma}}_t$ and its law of motion. TPI starts the economy at any initial distribution of savings $\bm{\hat{\Gamma}}_0$ and solves for its equilibrium time path over $T$ periods to the steady-state distribution $\bm{\bar{\Gamma}}_T$.

  The first step is to assume an initial transition path for aggregate capital $\mathbf{\hat{K}}^i = \left\{\hat{K}_1^i,\hat{K}_2^i,...\hat{K}_T^i\right\}$ such that $T$ is sufficiently large to ensure that $\bm{\hat{\Gamma}}_T = \bar{\bm{\Gamma}}$ and $\hat{K}_T^i\left(\bm{\Gamma}_T\right) = \bar{K}\left(\bar{\bm{\Gamma}}\right)$. The superscript $i$ is an index for the iteration number. The transition path for aggregate capital determines the transition path for both the real wage $\bm{\hat{w}}^i = \left\{\hat{w}_1^i,\hat{w}_2^i,...\hat{w}_T^i\right\}$ and the real return on investment $\bm{r}^i = \left\{r_1^i,r_2^i,...r_T^i\right\}$. The exact initial distribution of capital in the first period $\bm{\hat{\Gamma}}_1$ can be arbitrarily chosen as long as it satisfies $\hat{K}_1^i = \frac{1}{J}\sum_{s=1}^{S}\sum_{j=1}^{J}\hat{\omega}_{s,1}\hat{b}_{j,s,1}$ according to market clearing condition \eqref{EqMktClrCapStat}. One could also first choose the initial distribution of savings $\bm{\hat{\Gamma}}_1$ and then choose an initial aggregate capital stock $\hat{K}_1^i$ that corresponds to that distribution. As mentioned earlier, the only other restriction on the initial transition path for aggregate capital is that it equal the steady-state level $\hat{K}_T^i = \bar{K}\left(\bm{\bar{\Gamma}}\right)$ by period $T$. \citet{EvansPhillips:2014} have shown that the initial guess for the aggregate capital stocks $\hat{K}_t^i$ for periods $1<t<T$ can take on almost any positive values.

  Given the initial capital distribution $\bm{\hat{\Gamma}}_1$ and the transition paths of aggregate capital $\bm{\hat{K}}^i = \left\{\hat{K}_1^i,\hat{K}_2^i,...\hat{K}_T^i\right\}$, the real wage $\bm{\hat{w}}^i = \left\{\hat{w}_1^i,\hat{w}_2^i,...\hat{w}_T^i\right\}$, and the real return to investment $\bm{r}^i = \left\{r_1^i,r_2^i,...r_T^i\right\}$, one can solve for the optimal labor supply for each type $j$ of $s=S$-aged agents in the last period of their lives $n_{j,S,1}=\phi_{j,S}(\hat{b}_{j,S,1},\hat{w}_1,r_1)$ using his one static Euler equation, which is the $s=S$ version of \eqref{EqEulerLabStat}.
  ...
  % \begin{tabular}{>{\normalsize}l| >{\normalsize}c >{\normalsize}c >{\normalsize}c >{\normalsize}c >{\normalsize}c >{\normalsize}c >{\normalsize}c >{\normalsize}c}
  %          & Initial & & & & & & & Implied \\
  %          & Capital & & \multicolumn{4}{c}{Implied distribution of labor} & & Capital \\
  %   Period & Path    & & \multicolumn{4}{c}{supply and savings $\bm{\hat{\Gamma}}_t$}   & & Path    \\
  %   \hline
  %   & & & & & & & & \\
  %   $t=1$ & $\hat{K}_1^i$ & $=$ & $\hat{b}_{2,1}$ & $\hat{b}_{3,1}$ & $\hat{b}_{4,1}$ & $\bullet$ & $=$ & $\hat{K}_1^i$ \\[10mm]
  %   $t=2$ & $\hat{K}_2^i$ & $\rightarrow$ & $n_{1,1},\hat{b}_{2,2}$ & $n_{2,1},\hat{b}_{3,2}$ & $n_{3,1},\hat{b}_{4,2}$ & $n_{4,1}$ & $\rightarrow$ & $\hat{K}_2^{i'}$ \\[10mm]
  %   $t=3$ & $\hat{K}_3^i$ & $\rightarrow$ & $n_{1,2},\hat{b}_{2,3}$ & $n_{2,2},\hat{b}_{3,3}$ & $n_{3,2},\hat{b}_{4,3}$ & $n_{4,2}$ & $\rightarrow$ & $\hat{K}_3^{i'}$ \\[10mm]
  %   $t=4$ & $\hat{K}_4^i$ & $\rightarrow$ & $n_{1,3},\hat{b}_{2,4}$ & $n_{2,3},\hat{b}_{3,4}$ & $n_{3,3},\hat{b}_{4,4}$ & $n_{4,3}$ & $\rightarrow$ & $\hat{K}_4^{i'}$ \\[10mm]
  %   $\quad\vdots$ & $\vdots$ & & $\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & & $\vdots$ \\[10mm]
  %   $t=T-2$ & $\hat{K}_{T-2}^i$ & $\rightarrow$ & $n_{1,T-3},\hat{b}_{2,T-2}$ & $n_{2,T-3},\hat{b}_{3,T-2}$ & $n_{3,T-3},\hat{b}_{4,T-2}$ & $n_{4,T-3}$ & $\rightarrow$ & $\hat{K}_{T-2}^{i'}$ \\[10mm]
  %   $t=T-1$ & $\hat{K}_{T-1}^i$ & $\rightarrow$ & $n_{1,T-2},\hat{b}_{2,T-1}$ & $n_{2,T-2},\hat{b}_{3,T-1}$ & $n_{3,T-2},\hat{b}_{4,T-1}$ & $n_{4,T-2}$ & $\rightarrow$ & $\hat{K}_{T-1}^{i'}$ \\[10mm]
  %   $t=T$ & $\hat{K}_{T}^i$ & $\rightarrow$ & $n_{1,T-1},\hat{b}_{2,T}$ & $n_{2,T-1},\hat{b}_{3,T}$ & $n_{3,T-1},\hat{b}_{4,T}$ & $n_{4,T-1}$ & $\rightarrow$ & $\hat{K}_T^{i'}$ \\[10mm]
  %   $t=T+1$ & $\hat{K}_{T+1}^i$ & $\rightarrow$ & $\bullet$ & $n_{2,T},\hat{b}_{3,T+1}$ & $n_{3,T},\hat{b}_{4,T+1}$ & $n_{4,T}$ & $\rightarrow$ & $\hat{K}_{T+1}^{i'}$ \\[10mm]
  %   $t=T+2$ & $\hat{K}_{T+2}^i$ & $\rightarrow$ & $\bullet$ & $\bullet$ & $n_{3,T+1},\hat{b}_{4,T+2}$ & $n_{4,T+1}$ & $\rightarrow$ & $\hat{K}_{T+2}^{i'}$ \\[10mm]
  %   $t=T+3$ & $\hat{K}_{T+3}^i$ & $\rightarrow$ & $\bullet$ & $\bullet$ & $\bullet$ & $n_{4,T+2}$ & $\rightarrow$ & $\hat{K}_{T+3}^{i'}$ \\
  % \end{tabular}

  Once the set of lifetime saving decisions has been computed for all individuals alive in $1\leq t\leq T$, we use the household decisions to compute a new implied time path of the aggregate capital stock. The implied path of the aggregate capital stock $\bm{\hat{K}}^{i'}=\{\hat{K}_1^i,\hat{K}_2^{i'},...\hat{K}_T^{i'}\}$ in general does not equal the initial path of the aggregate capital stock $\bm{\hat{K}}^{i}=\{\hat{K}_1^i,\hat{K}_2^{i},...\hat{K}_T^{i}\}$ that was used to compute the household savings decisions $\bm{\hat{K}}^{i'}\neq\bm{\hat{K}}^i$.

  Let $\norm{\:\cdot\:}$ be a norm on the space of time paths of the aggregate capital stock $\bm{\hat{K}}\in\mathcal{K}\subset\mathbb{R}_{++}^T$. Then the fixed point necessary for the equilibrium transition path from Definition \ref{DefEquilNonSS} has been found when the distance between $\bm{\hat{K}}^{i'}$ and $\bm{\hat{K}}^{i}$ is arbitrarily close to zero.
  \begin{equation}\label{EqTPIconverge}
    \norm{\bm{\hat{K}}^{i'} - \bm{\hat{K}}^{i}} \leq \ve \quad\text{for}\quad \ve>0
  \end{equation}
  If the fixed point has not been found $\norm{\bm{\hat{K}}^{i'} - \bm{\hat{K}}^{i}} > \ve$, then a new transition path for the aggregate capital stock is generated as a convex combination of $\bm{\hat{K}}^{i'}$ and $\bm{\hat{K}}^{i}$.
  \begin{equation}\label{EqTPInewpath}
    \bm{\hat{K}}^{i+1} = \nu\bm{\hat{K}}^{i'} + (1-\nu)\bm{\hat{K}}^{i} \quad\text{for}\quad \nu\in(0,1)
  \end{equation}
  ...
  In essence, the TPI method iterates on individual beliefs about the time path of prices represented by a time path for the aggregate capital stock $\bm{\hat{K}}^i$ until a fixed point in beliefs is found that are consistent with the transition path implied by optimization based on those beliefs.

  The following are the steps for computing a stationary non-steady-state equilibrium time path for the economy.
  \begin{enumerate}
    \item Using the parameterization from the steady-state computation, and choose the value for $T$ at which the non-steady-state transition path should have converged to the steady state. $T$ should be at least as large as the number of periods it takes the population to reach its steady state $\bm{\bar{\omega}}$ as described in Appendix \ref{AppPopGrowth}.

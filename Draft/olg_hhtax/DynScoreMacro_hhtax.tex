...
\section{Model with Household Taxes}\label{SecModel}

  This is the deterministic OLG model with population growth and labor augmenting technological change in which households live $S$ periods and are one of $J$ ability types. The ability process is calibrated to match the wage distribution by age in the United States, and labor is endogenously supplied by individuals. The production side of the economy is characterized by a unit measure of identical, perfectly competitive firms. We carefully characterize the household tax structure of the economy.
  ...
  \subsection{Individual problem}\label{SecIndProb}
    ...
    \begin{equation}\label{EqBC}
      \begin{split}
        c_{j,s,t} \leq (1-\tau_c)\Bigl[(1+r_t)b_{j,s,t} + w_t e_{j,s}n_{j,s,t} - b_{j,s+1,t+1} - T_{j,s,t}^P - T_{j,s,t}^I\Bigr] \\
        \quad \forall j,s,t \quad\text{and}\quad b_{j,1,t},b_{j,S+1,t} = 0
      \end{split}
    \end{equation}
    where $\tau_c$ is a consumption tax. The two final terms on the right-hand-side of \eqref{EqBC} are, respectively, payroll tax and income tax functions. Note that the price of consumption is normalized to one, so $w_t$ is the real wage and $r_t$ is the real net interest rate.

    In addition to the budget constraints in each period, the utility function imposes nonnegative consumption through infinite marginal utility and individual labor and leisure must be nonnegative $n_{j,s,t},l_{j,s,t}\geq 0$. We allow the possibility for individual agents to borrow $b_{j,s,t}<0$ for some $j$ and $s$ in period $t$. However, the borrowing must satisfy a series of individual feasibility constraints as well as a strict constraint that the aggregate capital stock $K_t>0$ be positive in every period.\footnote{We describe these constraints in detail in Appendix \ref{AppBorConstr}.}

    The payroll tax function $T_{j,s,t}^P$ is meant to mimic the funding of the U.S. Social Security system and has the following form,
    \begin{equation}\label{EqTP}
      T_{j,s,t}^P =
        \begin{cases}
          \tau_P w_t e_{j,s}n_{j,s,t} \quad\text{if}\quad w_t e_{j,s}n_{j,s,t} < e^{g_y t}\hat{y}_P \quad\text{and}\quad s<R \\
          \tau_P e^{g_y t}\hat{y}_P \quad\quad\:\:\:\text{if}\quad w_t e_{j,s}n_{j,s,t} \geq e^{g_y t}\hat{y}_P \quad\text{and}\quad s<R \\
          -\theta_P e^{g_y t}\hat{a}_{j,s,t} \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\text{if}\quad s\geq R
        \end{cases} \quad\forall j,s,t
    \end{equation}
    where $\tau_P$ is the payroll tax rate on all labor income up to the growing income level $e^{g_y t}\hat{y}_P$ for pre-retirment ages $s<R$ and $e^{g_y t}$ is the deterministic cummulative growth in the economy since $t=0$.\footnote{This growth rate is labor augmenting technological change and is introduced in Section \ref{SecFirms}.} We assume that all individuals retire at age $R\leq S$, but that age $R$ is only the age that the social security benefits start coming. Individuals age $s\geq R$ still endogenously choose how much labor to supply. $\theta_P$ is the replacement rate of the social security benefit, which is a percent of the growth-adjusted average index of monthly earnings (AIME) $e^{g_y t}\hat{a}_{j,s,t}$. The AIME is calculated as the lifetime average earnings in each period up to age $s=R-1$.
    \begin{equation}\label{EqAIME}
      e^{g_y(t+1)}\hat{a}_{j,s+1,t+1} =
        \begin{cases}
          w_t e_{j,s}n_{j,s,t} \quad\quad\quad\quad\quad\quad\quad\quad\quad\text{if}\quad s=1 \\
          \left(\frac{s-1}{s}\right)e^{g_y t}\hat{a}_{j,s,t} + \frac{1}{s}w_t e_{j,s}n_{j,s,t} \quad 1<s<R \\
          e^{g_y t}\hat{a}_{j,s,t} \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\text{if}\quad s\geq R
        \end{cases} \quad\forall j,s,t
    \end{equation}

    The labor income tax function $T_{j,s,t}^I$ is a piecewise linear step function of $M$ marginal tax rates $\{\tau_{I,m}\}_{m=1}^M$ on ascending income brackets defined by $M-1$ labor income brackets $\{\hat{y}_{I,m}\}_{m=1}^{M-1}$. The number of marginal income tax rates in the U.S. is currently $M=7$. Figure \ref{FigUSmargtaxrates} plots the marginal tax rates with cutoffs, for a married individual filing jointly. Table \ref{TabUSmargtaxrates} shows the U.S. marginal tax rates and cutoffs for the four different categories of filer types for the year 2013. If we define an individual's labor income as $y_{j,s,t}\equiv w_t e_{j,s}n_{j,s,t}$, the labor income tax function $T_{j,s,t}^I$ has the following form.
    \begin{equation}\label{EqTI}
      \begin{split}
        T_{j,s,t}^I = &\tau_{I,1}\min\{e^{g_y t}\hat{y}_{I,1},y_{j,s,t}\} + \tau_{I,2}\max\Bigl\{0,\min\{e^{g_y t}\hat{y}_{I,2},y_{j,s,t}\} - e^{g_y t}\hat{y}_{I,1}\Bigr\} + ... \\
        &\quad\quad\tau_{I,M-1}\max\Bigl\{0,\min\{e^{g_y t}\hat{y}_{I,M-1},y_{j,s,t}\} - e^{g_y t}\hat{y}_{I,M-2}\Bigr\} + \\
        &\quad\quad\quad\quad\tau_{I,M}\max\{0, y_{j,s,t}-e^{g_y t}\hat{y}_{I,M-1}\} \quad\forall j,s,t
      \end{split}
    \end{equation}
    The growth rate terms $e^{g_y t}$ in \eqref{EqTI} signify that the income bracket cutoffs are growing at the rate of the economy.

    \begin{figure}[htbp]\centering \captionsetup{width=4.0in}
      \caption{\label{FigUSmargtaxrates}\textbf{U.S. marginal tax rate schedule for married filing jointly, 2013}}
      \fbox{\resizebox{4.0in}{2.8in}{\includegraphics{images/USmargtaxrates.pdf}}}
    \end{figure}

    \begin{table}[htbp] \centering \captionsetup{width=6.0in}
    \caption{\label{TabUSmargtaxrates}\textbf{U.S. marginal tax rate schedule by filer type, 2013}}
      \begin{threeparttable}
      \begin{tabular}{>{\footnotesize}c |>{\footnotesize}c >{\footnotesize}c >{\footnotesize}c >{\footnotesize}c}
        \hline\hline
        Marginal & \multicolumn{4}{c}{\footnotesize{Income brackets by filer type}} \\
        \cline{2-5}
        tax &  & Married filing jointly & Married filing & Head of \\
        rate & Single & or qualified widow(er) & separately & household \\
        \hline
      ...
      \end{tabular}
      \begin{tablenotes}
        \scriptsize{\item[]Source: U.S. Internal Revenue Service.}
      \end{tablenotes}
      \end{threeparttable}
    \end{table}

    The labor income cutoff $\hat{y}_P$ and the change in tax structure at $s=R$ in the payroll tax \eqref{EqTP} as well as the discrete jumps in marginal tax rates in the income tax \eqref{EqTI} render the household's optimization problem every period highly nonconvex. And the kinks in the intertemporal budget constraint multiply for younger households with more periods until death. For this reason, a dynamic programming approach to the household optimization problem is necessary. We solve for optimal household decisions by backward induction from the last period of an individual's life to the first period.

    In order to perform this functional analysis, we must stationarize the characterizing equations for household optimization. The individual (per-capita) endogenous variables in this section are growing at the rate of labor augmenting technological progress $e^{g_y}$, the role of which is described in Section \ref{SecFirms}. Table \ref{TabHHstatVars} defines the stationarized versions of the household variables.

    \begin{table}[htbp] \centering \captionsetup{width=2.9in}
    \caption{\label{TabHHstatVars}\textbf{Stationary household variable definitions}}
      \begin{threeparttable}
      \begin{tabular}{>{\small}c >{\small}c >{\small}c}
        \hline\hline
        & & \\[-4mm]
        $\hat{c}_{j,s,t}\equiv \frac{c_{j,s,t}}{e^{g_y t}}$ & $\hat{b}_{j,s,t}\equiv\frac{b_{j,s,t}}{e^{g_y t}}$ & $\hat{w}_t\equiv\frac{w_t}{e^{g_y t}}$ \\[2mm]
        $\hat{T}_{j,s,t}^P\equiv \frac{T_{j,s,t}^P}{e^{g_y t}}$ & $\hat{T}_{j,s,t}^I\equiv \frac{T_{j,s,t}^I}{e^{g_y t}}$ & $\hat{y}_{j,s,t}\equiv\frac{y_{j,s,t}}{e^{g_y t}}$ \\[2mm]
        \hline\hline
      \end{tabular}
      \begin{tablenotes}
    ...
    \noindent The stationarized version of the household budget constraint \eqref{EqBC} is the following,
    \begin{equation}\label{EqBCstat}
      \begin{split}
        \hat{c}_{j,s,t} \leq (1-\tau_c)\Bigl[(1+r_t)\hat{b}_{j,s,t} + \hat{w}_t e_{j,s}n_{j,s,t} - e^{g_y}\hat{b}_{j,s+1,t+1} - \hat{T}_{j,s,t}^P - \hat{T}_{j,s,t}^I\Bigr] \\
        \quad \forall j,s,t \quad\text{and}\quad \hat{b}_{j,1,t},\hat{b}_{j,S+1,t} = 0
      \end{split}
    \end{equation}
    where the stationary versions of the household tax policies \eqref{EqTP}, \eqref{EqAIME}, and \eqref{EqTI} are the following.
    \begin{equation}\label{EqTPstat}
      \hat{T}_{j,s,t}^P =
        \begin{cases}
          \tau_P\hat{w}_t e_{j,s}n_{j,s,t} \quad\text{if}\quad \hat{w}_t e_{j,s}n_{j,s,t} < \hat{y}_P \quad\text{and}\quad s<R \\
          \tau_P\hat{y}_P \quad\quad\quad\quad\:\text{if}\quad \hat{w}_t e_{j,s}n_{j,s,t} \geq \hat{y}_P \quad\text{and}\quad s<R \\
          -\theta_P \hat{a}_{j,s,t} \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\text{if}\quad s\geq R
        \end{cases} \quad\forall j,s,t
    \end{equation}
    \begin{equation}\label{EqAIMEstat}
      e^{g_y}\hat{a}_{j,s+1,t+1} =
        \begin{cases}
          \hat{w}_t e_{j,s}n_{j,s,t} \quad\quad\quad\quad\quad\quad\quad\quad\:\:\text{if}\quad s=1 \\
          \left(\frac{s-1}{s}\right)\hat{a}_{j,s,t} + \left(\frac{1}{s}\right)\hat{w}_t e_{j,s}n_{j,s,t} \quad 1<s<R \\
          \hat{a}_{j,s,t} \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\text{if}\quad s\geq R
        \end{cases} \quad\forall j,s,t
    \end{equation}
    \begin{equation}\label{EqTIstat}
      \begin{split}
        \hat{T}_{j,s,t}^I = &\tau_{I,1}\min\{\hat{y}_{I,1},\hat{y}_{j,s,t}\} + \tau_{I,2}\max\Bigl\{0,\min\{\hat{y}_{I,2},\hat{y}_{j,s,t}\} - \hat{y}_{I,1}\Bigr\} + ... \\
        &\quad\quad\tau_{I,M-1}\max\Bigl\{0,\min\{\hat{y}_{I,M-1},\hat{y}_{j,s,t}\} - \hat{y}_{I,M-2}\Bigr\} + \\
        &\quad\quad\quad\quad\tau_{I,M}\max\{0, \hat{y}_{j,s,t}-\hat{y}_{I,M-1}\} \quad\forall j,s,t
      \end{split}
    \end{equation}

    In the last period of an individual's life $s=S$, one simply chooses how much to work and then consumes all his after-tax income. The household optimization problem can be written as an age-dependent Bellman equation.
    \begin{equation}\label{EqHHSmaxprob}
      \begin{split}
        &V_{j,S}\bigl(\hat{b}_{j,S,t},\hat{a}_{j,S,t},\hat{w}_t,r_t\bigr) = \\
        &\max_{n_{j,S,t}}\quad\frac{\biggl((1-\tau_c)\Bigl[(1+r_t)b_{j,S,t} + w_t e_{j,S}n_{j,S,t} + \theta_P e^{g_y t}\hat{a}_{j,S,t} - T_{j,S,t}^I\bigr]\Biggr)^{1-\sigma} - 1}{1-\sigma} + \\
        &\quad\quad\quad\quad\quad\chi e^{g_y t(1-\sigma)}\frac{(\tilde{l} - n_{j,S,t})^{1-\eta}}{1-\eta} \quad\forall j,t
      \end{split}
    \end{equation}
    For the age-$S$ problem \eqref{EqHHSmaxprob}, the optimization is nonconvex due to the piecewise linear labor income tax $T_{j,S,t}^I$. Notice that the payroll tax has switched to a fixed benefit because $s\geq R$.
...
\section{Calibration of ability process}\label{AppAbilCalib}
  ...
  The data used to calibrate the ability types were obtained from the Current Population Survey.\footnote{U.S. Census Bureau, Dataferret, Current Population Survey, 2014. The variables $PRTAGE$, $PTERNHLY$, and $PWCMPWGT$ were used for the age, hourly wage rate, and population weight of individuals, respectively. \\ [-2pt]} Individuals younger than 20 and older than 79 are dropped from the data. This is due to the extremely small amount of observations for ages outside of those bounds. Due to a limited number of observations in the survey who included their hourly wage, data was taken from the months of January, February, March, April, and May 2014.  Population weights were also used to obtain the correct percentile groups of individuals.  The income levels for the $J$ ability types were then calculated for each month, and then an average was taken of the five calibrations of the ability types in order to produce a final calibration to be used in the model. Figure \ref{FigIncome} shows this income distribution across age and ability type.
    ...
    \begin{figure}[htb]\centering \captionsetup{width=4.0in}
      ...
      \fbox{\resizebox{4.0in}{2.8in}{\includegraphics{images/income.png}}}
    \end{figure}
...
\section{Characteristics of exogenous population growth assumptions}\label{AppPopGrowth}
  ...
  \begin{figure}[htbp]\centering \captionsetup{width=4.0in}
    ...
    \fbox{\resizebox{4.0in}{2.8in}{\includegraphics{images/InitPopDist.pdf}}}
  \end{figure}

  \begin{figure}[htbp]\centering \captionsetup{width=4.0in}
    \caption{\label{FigFertRates}\textbf{Fertility rates $f_s$ for $S=60$}}
    \fbox{\resizebox{4.0in}{2.8in}{\includegraphics{images/FertRates.pdf}}}
  \end{figure}

  \begin{figure}[htbp]\centering \captionsetup{width=4.0in}
    \caption{\label{FigMortRates}\textbf{Mortality rates $\rho_s$ for $S=60$}}
    \fbox{\resizebox{4.0in}{2.8in}{\includegraphics{images/MortRates.pdf}}}
  \end{figure}

  \begin{figure}[htbp]\centering \captionsetup{width=4.0in}
    \caption{\label{FigImmigRates}\textbf{Immigration rates $i_s$ for $S=60$}}
    \fbox{\resizebox{4.0in}{2.8in}{\includegraphics{images/ImmigRates.pdf}}}
  \end{figure}
  ...
  \begin{figure}[htbp]\centering \captionsetup{width=4.0in}
    ...
    \fbox{\resizebox{4.0in}{2.8in}{\includegraphics{images/PopPath.pdf}}}
  \end{figure}

  \begin{figure}[htbp]\centering \captionsetup{width=4.0in}
    \caption{\label{FigSSpopdist}\textbf{Steady-state population percent distribution by age $\bm{\bar{\omega}}$}}
    \fbox{\resizebox{4.0in}{2.8in}{\includegraphics{images/SSpopdist.pdf}}}
  \end{figure}
  \clearpage
...
\newpage
\bibliography{DynScoreMacro_hhtax}

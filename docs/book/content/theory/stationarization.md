...
## Stationarized Firms Equations
  ...
  :label: EqStnrzProfit
    ...
    &\qquad\qquad\quad \bigl(r_t + \delta_{M,t}\bigr)\hat{K}_{m,t} + \left(\tau^{corp}_{m,t}\delta^\tau_{m,t} + \tau^{inv}_{m,t}\right)\hat{K}_{m,t} \quad\forall m,t
  ```
...
## Stationarized Government Equations
  ...
  ```{math}
  :label: EqStnrzGovRev
    \hat{Rev}_t &= \underbrace{\sum_{m=1}^M\Bigl[\tau^{corp}_{m,t}\bigl(p_{m,t}\hat{Y}_{m,t} - \hat{w}_t\hat{L}_t\bigr) - \left(\tau^{corp}_{m,t}\delta^\tau_{m,t} + \tau^{inv}_{m,t}\right)\hat{K}_{m,t}\Bigr]}_{\text{corporate tax revenue}} \\

...
def SS_solver(
    bmat, nmat, r, w, Y, BQ, TR, factor, p, client, fsolve_flag=False
):
    """
    ...
    etr_ss = tax.ETR_income(
        r_p_ss, wss, bssmat_s, nssmat, factor, p.e, etr_params_3D, p
    )
...
def run_SS(p, client=None):
    ...
    if p.baseline:
        ...
        if p.baseline_spending:
            ...
            if p.use_zeta:
                guesses = (
                    [rguess, wguess, Yguess] + list([BQguess]) + [TRguess]
                )
            else:

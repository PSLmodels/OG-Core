...
def save_baseline_results(baseline_name: Optional[str] = None):
    ...
    baseline_file = os.path.join(
        BENCHMARK_RESULTS_DIR, f"baseline_{baseline_name}.json"
    )
    ...
    print(
        f"Saved {len(baseline_data['results'])} benchmark results as baseline: {baseline_file}"
    )
...
def load_baseline_results(
    ...
    if baseline_name:
        ...
        if baseline_file not in baseline_files:
            print(
                f"Baseline '{baseline_name}' not found. Available: {baseline_files}"
            )
            return None
    else:
...
def compare_with_baseline(baseline_name: Optional[str] = None):
    ...
    print("-" * 80)
    print(
        f"{'Test':<25} {'Scheduler':<15} {'Time Change':<12} {'Memory Change':<15}"
    )
    print("-" * 80)
    ...
    for key in sorted(common_keys):
        ...
        time_change = (
            (current.compute_time - baseline.compute_time)
        ...
        time_indicator = (
            "🟢" if time_change < -5 else "🔴" if time_change > 5 else "🟡"
        )
        memory_indicator = (
            "🟢" if memory_change < -5 else "🔴" if memory_change > 5 else "🟡"
        )

        print(
            f"{key[0]:<25} {key[1]:<15} {time_str:<12} {memory_str:<15} {time_indicator}{memory_indicator}"
        )
    ...
    significant_memory_improvements = [
        m for m in memory_improvements if m < -10
    ]
    significant_memory_regressions = [m for m in memory_improvements if m > 10]

    if significant_time_improvements:
        print(
            f"🟢 {len(significant_time_improvements)} tests showed >10% time improvement"
        )
    if significant_time_regressions:
        print(
            f"🔴 {len(significant_time_regressions)} tests showed >10% time regression"
        )
    if significant_memory_improvements:
        print(
            f"🟢 {len(significant_memory_improvements)} tests showed >10% memory improvement"
        )
    if significant_memory_regressions:
        print(
            f"🔴 {len(significant_memory_regressions)} tests showed >10% memory regression"
        )
...
Examples:
  ...
  python run_benchmarks.py --compare-baseline       # Compare with most recent baseline
    ...
    if args.compare_baseline is not None:
        baseline_name = (
            args.compare_baseline if args.compare_baseline else None
        )

...
def test_token_file_is_never_briefly_world_readable(
    isolated_token_env, monkeypatch
):
    """The token is a credential, so the file is created 0600 rather than
    created at the umask default and tightened afterwards. Asserting on the
    mode seen at write time catches a regression to chmod-after."""
    import stat
    ...
    def spy(handle, *args, **kwargs):
        seen["mode_at_write"] = stat.S_IMODE(os.fstat(handle).st_mode)
        return real_fdopen(handle, *args, **kwargs)

    monkeypatch.setattr(os, "fdopen", spy)
    monkeypatch.setattr(
        demographics.getpass, "getpass", lambda *a, **k: "a_secret"
    )
    assert demographics.un_token_cli(["set"]) == 0

    path = demographics.un_token_path()
    assert seen["mode_at_write"] == 0o600  # already restricted when written
    assert stat.S_IMODE(os.stat(path).st_mode) == 0o600


def test_existing_token_file_permissions_are_tightened(
    isolated_token_env, monkeypatch
):
    """A creation mode does not apply to a file that already exists, so the
    chmod still has to run for a file left behind with loose permissions."""
    import stat

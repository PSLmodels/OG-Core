...
def create_realistic_micro_data(num_records_per_year: int = 2000, num_years: int = 3):
    ...
    for year_idx, year in enumerate(range(2021, 2021 + num_years)):
        ...
        total_capital_income = interest_income + dividend_income + capital_gains
        ...
        marginal_rates = np.where(total_income < 20000, 0.10,
                          np.where(total_income < 50000, 0.15,
                           np.where(total_income < 100000, 0.22,
                            np.where(total_income < 200000, 0.28, 0.32))))
        
        # Effective tax rates (lower than marginal due to deductions, etc.)
        effective_rates = marginal_rates * 0.7 * (total_income / (total_income + 10000))
        ...
        mtr_capital = marginal_rates * 0.8 * np.random.normal(1.0, 0.05, n_records)  # Usually lower
        ...
        data = pd.DataFrame({
            ...
            'MARS': np.random.choice([1, 2, 3, 4], n_records, p=[0.4, 0.45, 0.1, 0.05]),
            'FLPDYR': year,
...
class TestRealTaxFuncBenchmarks:
    ...
    def medium_real_data(self):
        ...
        return create_realistic_micro_data(num_records_per_year=1500, num_years=3)
    ...
    def run_real_benchmark(
        ...
        for year_data in micro_data.values():
            data_size_mb += year_data.memory_usage(deep=True).sum() / 1024 / 1024
            ...
            with MemoryTracker() as mem_tracker:
                with timer() as get_time:
                    try:
                        ...
                        expected_keys = ['tfunc_etr_params_S', 'tfunc_mtrx_params_S', 'tfunc_mtry_params_S']
                        for key in expected_keys:
                            if key not in result:
                                raise ValueError(f"Missing expected result key: {key}")
    ...
    def test_real_small_no_client(self, small_real_data):
        ...
        print(f"Real small (no client): {result.compute_time:.3f}s, {result.peak_memory_mb:.1f}MB peak")
    ...
    def test_real_small_threaded_client(self, small_real_data):
        ...
        try:
            ...
            print(f"Real small (threaded): {result.compute_time:.3f}s, {result.peak_memory_mb:.1f}MB peak")
            assert result.success, f"Real threaded benchmark failed: {result.error_message}"
    ...
    @pytest.mark.real
    @pytest.mark.distributed  
    @pytest.mark.skipif(platform.system() == "Windows", reason="Multiprocessing issues on Windows")
    def test_real_small_process_client(self, small_real_data):
        ...
        try:
            ...
            print(f"Real small (processes): {result.compute_time:.3f}s, {result.peak_memory_mb:.1f}MB peak")
            assert result.success, f"Real process benchmark failed: {result.error_message}"
    ...
    def test_real_medium_comparison(self, medium_real_data):
        ...
        try:
            for config_name, client in configs:
                ...
                print(f"Real medium ({config_name}): {result.compute_time:.3f}s, {result.peak_memory_mb:.1f}MB")
        ...
        assert len(successful_results) > 0, "All real medium benchmark configurations failed"
        
        # Report performance comparison
        if len(successful_results) > 1:
            fastest = min(successful_results, key=lambda x: x.compute_time)
            print(f"Fastest configuration: {fastest.scheduler} ({fastest.compute_time:.3f}s)")
    ...
    def test_real_memory_efficiency(self, small_real_data, medium_real_data):
        ...
        for name, data in datasets:
            ...
            memory_efficiency = result.peak_memory_mb / result.data_size_mb if result.data_size_mb > 0 else float('inf')
            print(f"Real memory {name}: {result.data_size_mb:.1f}MB data -> {result.peak_memory_mb:.1f}MB peak (efficiency: {memory_efficiency:.2f})")
        ...
        if len(results) >= 2 and all(r.success for r in results):
            ...
            print(f"Memory scaling: {memory_ratio:.2f}x memory for {data_ratio:.2f}x data")
            
            # Memory should not scale too poorly (allow some overhead for processing)
            assert memory_ratio <= data_ratio * 3, f"Poor memory scaling: {memory_ratio:.2f}x memory for {data_ratio:.2f}x data"
...
def test_platform_specific_optimal_config():
    ...
    test_data = create_realistic_micro_data(num_records_per_year=800, num_years=2)
    ...
    if platform.system() != "Windows":
        try:
            cluster = LocalCluster(n_workers=2, threads_per_worker=1, processes=True, silence_logs=True)
            client = Client(cluster)
            configurations.append(("distributed_processes", client, "processes"))
        except Exception as e:
    ...
    if successful_results:
        ...
        print(f"Time: {optimal.compute_time:.3f}s, Memory: {optimal.peak_memory_mb:.1f}MB")
        ...
        rec_file = os.path.join(os.path.dirname(__file__), "benchmark_results", 

...
\implies & K_{i,u+1} = \gamma_{i}X_{i,u+1}A_{i,u+1}^{1-\epsilon_{i}} \left(\frac{p_{i,u+1}}{p^{k}_{i,u}(1+r_{u+1}) - p^{k}_{i,u+1}(1-\delta)} \right)^{\epsilon_{i}} = K(p_{i,u+1},p^{k}_{i,u},p^{k}_{i,u+1},r_{u+1},X_{i,u+1})
\end{split}
\end{equation}
...
\implies r_{t} &= \frac{DIV_{i,t} + \sum_{u=t}^{\infty}\prod_{\nu=t}^{u}\left(\frac{1}{1+r_{\nu}}\right)\left[DIV_{i,u}-VN_{i,u}\right] -\sum_{u=t+1}^{\infty}\prod_{\nu=t}^{u}\left(\frac{1}{1+r_{\nu}}\right)\left[DIV_{i,u}-VN_{i,u}\right]}{V_{i,t}} ...\\
...
\implies &  1+r_{t} = \frac{ p_{i,t}X_{i,u} - w_{t}EL(p_{i,t},w_{u},X_{i,t}) - p^{k}_{i,u}I_{i,t}}{V_{i,t}} \\
\implies &  1+r_{t} = \frac{ p_{i,t}X_{i,u} - w_{t}EL(p_{i,t},w_{u},X_{i,t})}{V_{i,t}}... \\
 ...
 \implies &  1+r_{t} = \frac{p_{i,t}X_{i,t} - w_{u}\left( \frac{ p_{i,t}}{w_{t}}\right)^{\epsilon_{i}}A_{i,t}^{\epsilon_{i}-1}X_{i,t}}{V_{i,t}} ...\\ 
...
This means we have an equations written in terms of factor prices, output prices, capital prices, output, and parameters.  This will be the equation we use to solve for prices, but we need two more pieces. 
...
Second, we need to solve for $V_{i,t}$ in terms of factor prices, output prices, capital prices, output, and parameters.  To do this, we will use the result of \citet{Hayashi1982}.  What Hayashi shows is that with quadratic adjustment costs to changing the capital stock (which we have hear - albeit the costs are zero in this simple model) and a production function that is proportional to the capital stock (which we have since we have a CRS production function), the the firm's marginal q, $q_{i,u}$, is related to the firms average q, $Q_{i,u}$ in the  following way:

\begin{equation}
q_{i,t} = \frac{V_{i,t}-p_{i,t}X-{i,t}}{K_{i,t}}, \quad Q_{i,t} = \frac{V_{i,t}}{K_{i,t}}
\end{equation}
...
\begin{equation}
\label{eqn:solve_v1}
V_{i,t} = q_{i,t}K_{i,t} + p_{i,t}X_{i,t}
\end{equation}

Using Equations \ref{eqn:lagrange_foc_i} and \ref{eqn:lagrange_foc_k} (the first order conditions for investment and one period ahead capital), we can rewrite \ref{eqn:solve_v1} as

\begin{equation}
\label{eqn:solve_v2}
\begin{split}
& V_{i,t} = q_{i,t}K_{i,t} + p_{i,t}X_{i,t} \\
\implies & V_{i,t} =  \left(\frac{1}{1+r_{t+1}}\right) \left[ p_{i,t+1}\frac{\partial F(K_{i,t+1},EL_{i,t+1})}{\partial K_{i,t+1}}+ p^{k}_{i,t+1}(1-\delta)\right]K_{i,t} + p_{i,t}X_{i,t} \\
\implies & V_{i,t} =  \left(\frac{1}{1+r_{t+1}}\right) \left[ p_{i,t+1}\left(\frac{p^{k}_{i,t}}{p_{i,t+1}}(1+r_{t+1}) - \frac{p^{k}_{i,t+1}}{p_{i,t+1}}(1-\delta) \right)+ p^{k}_{i,t+1}(1-\delta)\right]K_{i,t} + p_{i,t}X_{i,t} \\
\implies & V_{i,t} =   \left(\frac{1}{1+r_{t+1}}\right) \left[ p^{k}_{i,t}(1+r_{t+1}) - p^{k}_{i,t+1}(1-\delta)+ p^{k}_{i,t+1}(1-\delta)\right]K_{i,t} + p_{i,t}X_{i,t} \\
\implies & V_{i,t} =   \left(\frac{1}{1+r_{t+1}}\right) \left[ p^{k}_{i,t}(1+r_{t+1})\right]K_{i,t} + p_{i,t}X_{i,t} \\
\implies & V_{i,t} =   p^{k}_{i,t}K_{i,t} + p_{i,t}X_{i,t} \\
\implies & V_{i,t} =   p^{k}_{i,t}\gamma X_{i,t}A_{i,t}^{1-\epsilon_{i}}\left(\frac{p_{i,t}}{p^{k}_{i,t-1}(1+r_{t})-p^{k}_{i,t}(1-\delta)} \right)^{\epsilon_{i}} + p_{i,t}X_{i,t}, \\
\end{split}
\end{equation}

\noindent\noindent where the third step follows from Equation \ref{eqn:opt_inv2}.
...
\begin{split}
&  1+r_{t} = \frac{p_{i,t}X_{i,t} - w_{u}\left( \frac{ p_{i,t}}{w_{t}}\right)^{\epsilon_{i}}A_{i,t}^{\epsilon_{i}-1}X_{i,t}}{V_{i,t}} ...\\ 
 & \quad - \frac{p^{k}_{i,t}\left( \gamma_{i}X_{i,t+1}A_{i,t+1}^{1-\epsilon_{i}} \left(\frac{p_{i,t+1}}{p^{k}_{i,t}(1+r_{t+1}) - p^{k}_{i,t+1}(1-\delta)} \right) - (1-\delta) \gamma_{i}X_{i,t}A_{i,t}^{1-\epsilon_{i}} \left(\frac{p_{i,t}}{p^{k}_{i,t-1}(1+r_{t}) - p^{k}_{i,t}(1-\delta)} \right)\right)}{V_{i,t}} \\
 \implies & 1+r_{t} = \frac{p_{i,t}X_{i,t} - w_{u}\left( \frac{ p_{i,t}}{w_{t}}\right)^{\epsilon_{i}}A_{i,t}^{\epsilon_{i}-1}X_{i,t}}{p^{k}_{i,t}\gamma X_{i,t}A_{i,t}^{1-\epsilon_{i}}\left(\frac{p_{i,t}}{p^{k}_{i,t-1}(1+r_{t})-p^{k}_{i,t}(1-\delta)} \right)^{\epsilon_{i}} + p_{i,t}X_{i,t}} ...\\ 
 & \quad - \frac{p^{k}_{i,t}\left( \gamma_{i}X_{i,t+1}A_{i,t+1}^{1-\epsilon_{i}} \left(\frac{p_{i,t+1}}{p^{k}_{i,t}(1+r_{t+1}) - p^{k}_{i,t+1}(1-\delta)} \right) - (1-\delta) \gamma_{i}X_{i,t}A_{i,t}^{1-\epsilon_{i}} \left(\frac{p_{i,t}}{p^{k}_{i,t-1}(1+r_{t}) - p^{k}_{i,t}(1-\delta)} \right)\right)}{p^{k}_{i,t}\gamma X_{i,t}A_{i,t}^{1-\epsilon_{i}}\left(\frac{p_{i,t}}{p^{k}_{i,t-1}(1+r_{t})-p^{k}_{i,t}(1-\delta)} \right)^{\epsilon_{i}} + p_{i,t}X_{i,t}} \\
\end{split}
\end{equation}
...
\begin{split}
& \bar{q}_{i} = \frac{\bar{V}_{i}-\bar{p}_{i}\bar{X}_{i}}{\bar{K}_{i}} \\
\implies & V_{i} = \bar{q}_{i}\bar{K}_{i} + \bar{p}_{i}\bar{X}_{i} \\
\implies & V_{i} = \bar{p}^{k}_{i}\bar{K}_{i} + \bar{p}_{i}\bar{X}_{i} \\
\implies & V_{i} = \bar{p}^{k}_{i}\gamma_{i}\bar{X}_{i}\bar{A}_{i}^{1-\epsilon_{i}} \left(\frac{\bar{p}_{i}}{\bar{p}^{k}_{i}(\bar{r}+\delta)} \right)^{\epsilon_{i}} + \bar{p}_{i}\bar{X}_{i} \\
\end{split}
\end{equation}
...
\implies & \bar{r} = \frac{ \bar{p}_{i}\bar{X}_{i}-\bar{w}\overline{EL}_{i}-\bar{p}^{k}_{i}\delta \bar{K}_{i}}{\bar{q}_{i}\bar{K}_{i} + \bar{p}_{i}\bar{X}_{i}} \\
\implies & \bar{r} = \frac{ \bar{p}_{i}\bar{X}_{i}-\bar{w}(1-\gamma_{i})\left(\left( \frac{ \bar{p}_{i}}{\bar{w}}\right)^{\epsilon_{i}}\bar{A}_{i}^{\epsilon_{i}-1}\bar{X}_{i} \right)-\bar{p}^{k}_{i}\delta \left(\gamma_{i}\bar{X}_{i}\bar{A}_{i}^{1-\epsilon_{i}} \left(\frac{\bar{p}_{i}}{\bar{p}^{k}_{i}(\bar{r}+\delta)} \right)^{\epsilon_{i}}\right)}{\bar{p}^{k}_{i}\gamma_{i}\bar{X}_{i}\bar{A}_{i}^{1-\epsilon_{i}} \left(\frac{\bar{p}_{i}}{\bar{p}^{k}_{i}(\bar{r}+\delta)} \right)^{\epsilon_{i}} + \bar{p}_{i}\bar{X}_{i}} \\
\implies & \bar{r} = \frac{ \bar{p}_{i}-\bar{w}(1-\gamma_{i})\left(\left( \frac{ \bar{p}_{i}}{\bar{w}}\right)^{\epsilon_{i}}\bar{A}_{i}^{\epsilon_{i}-1} \right)-\bar{p}^{k}_{i}\delta \left(\gamma_{i}\bar{A}_{i}^{1-\epsilon_{i}} \left(\frac{\bar{p}_{i}}{\bar{p}^{k}_{i}(\bar{r}+\delta)} \right)^{\epsilon_{i}}\right)}{\bar{p}^{k}_{i}\gamma_{i}\bar{A}_{i}^{1-\epsilon_{i}} \left(\frac{\bar{p}_{i}}{\bar{p}^{k}_{i}(\bar{r}+\delta)} \right)^{\epsilon_{i}} + \bar{p}_{i}} \\
\end{split}
\end{equation}

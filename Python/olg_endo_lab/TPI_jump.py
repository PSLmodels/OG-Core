...
def Euler2_jump(w, r, e, N_guess, K1_2, K2_2):
...
def Euler_Error(guesses, winit, rinit, t):
    ...
    e_euler2 = e[-(length+1):,j]

    get_N_zero = lambda x: Euler2_jump(w, r, e_euler2, x, K1_2, K2_2)
    N_guess = opt.fsolve(get_N_zero, N_guess)
...
while (TPIiter < TPImaxiter) and (TPIdist >= TPImindist):
    ...
    for j in xrange(J):
        for s in xrange(S-2):  # Upper triangle
            solutions = opt.fsolve(Euler_Error, list(initial_K[-(s+1):, j]), args=(winit, rinit, 0))
            K_vec = solutions
            K_mat[1:S, :, j] += np.diag(K_vec, S-(s+2))
            length = len(list(initial_K[-(s+1):, j]))
            N_guess = np.ones(length) * .95
            w = winit[0:length+1]
            r = rinit[0:length+1]
            e_euler2 = e[-(length+1):,j]
            K1_2 = np.array([0] + list(K_vec))
            K2_2 = np.array(list(K_vec) + [0])
            get_N_zero = lambda x: Euler2_jump(w, r, e_euler2, x, K1_2, K2_2)
            N_vec = opt.fsolve(get_N_zero, N_guess)
            N_mat[:S, :, j] += np.diag(N_vec, S-(s+2))

        for t in xrange(0, T):
            solutions = opt.fsolve(Euler_Error, list(initial_K[:, j]), args=(winit, rinit, t))
            K_vec = solutions
            K_mat[t+1:t+S, :, j] += np.diag(K_vec)
            N_guess = np.ones(S) * .95
            w = winit[t:t+S]
            r = rinit[t:t+S+1]
            e_euler2 = e[-S:,j]
            K1_2 = np.array([(initial_K[-(s+2), j])] + list(K_vec))
            K2_2 = np.array(list(K_vec) + [0])
            get_N_zero = lambda x: Euler2_jump(w, r, e_euler2, x, K1_2, K2_2)
            N_vec = opt.fsolve(get_N_zero, N_guess)

...
def create_tpi_params(a_tax_income, b_tax_income, c_tax_income,
    ...
    K0 = household.get_K(initial_b, omega_stationary[0])
...
def run_time_path_iteration(Kss, Lss, Yss, BQss, theta, income_tax_params, wealth_tax_params, ellipse_params, parameters, N_tilde, omega_stationary, K0, b_sinit, b_splus1init, L0, Y0, r0, BQ0, T_H_0, tax0, c0, initial_b, initial_n, factor_ss, tau_bq, chi_b, chi_n, get_baseline=False, **kwargs):
    ...
    cinit = household.get_cons(rinit[:T].reshape(T, 1, 1), b_s, winit[:T].reshape(T, 1, 1), e.reshape(1, S, J), n_mat[:T], BQinit[:T].reshape(T, 1, J), lambdas.reshape(1, 1, J), b_splus1, parameters, tax_path)

    print'Checking time path for violations of constaints.'
    for t in xrange(T):
        household.constraint_checker_TPI(b_mat[t, :-1, :], n_mat[
    ...
    output = {'Kpath_TPI':Kpath_TPI, 'b_mat':b_mat, 'cinit':cinit,

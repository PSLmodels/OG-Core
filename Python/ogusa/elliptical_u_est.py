...
def sumsq(params, *objs):
    ...
    CFE = ((n_grid)**(1+theta))/(1+theta)
    ellipse = b*((((n_grid/l_tilde)**upsilon))**(1/upsilon)) + k
    errors = CFE - ellipse
    ssqdev = (errors ** 2).sum()
...
def estimation(frisch, l_tilde):
    ...
    if graph == True:
        ...
        CFE_MU = -1.0*(n_grid)**theta #(l_tilde-n_grid)**theta
        ellipse_MU = -1.0*b_til * (1.0 / l_tilde) * ((1.0 - (n_grid / l_tilde) ** upsilon_til) ** ((1.0 / upsilon_til) - 1.0)) * (n_grid / l_tilde) ** (upsilon_til - 1.0)


        #b_til * (1.0 / l_tilde) * ((1.0 - (n_grid / l_tilde) ** upsilon_til) ** (

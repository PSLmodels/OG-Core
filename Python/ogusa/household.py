...
Household functions for taxes in the steady state and along the transition path.
...
def get_K(b, params):
    ...
    Inputs:
        ...
        params      = length 4 tuple, (pop_weights, lambdas, g_n, method)
        pop_weights = [S,] vector, population weights 
    ...
    pop_weights, lambdas, g_n, method = params 
    K_presum = b * pop_weights * lambdas
    if method == 'SS':
        K = K_presum.sum()
    elif method == 'TPI':
...
def get_BQ(r, b_splus1, params):
    ...
    Inputs:
        ...
        params      = length 5 tuple, (pop_weights, lambdas, rho, g_n, method)
        pop_weights = [S,] vector, population weights 
        lambdas     = [J,] vector, fraction in each lifetime income group 
    ...
    Returns: BQ
    '''
    pop_weights, lambdas, rho, g_n, method = params 

    BQ_presum = b_splus1 * pop_weights * rho * lambdas
    if method == 'SS':
        BQ = BQ_presum.sum(0)
    elif method == 'TPI':
        BQ = BQ_presum.sum(1)
...
def marg_ut_labor(n, params):
    ...
    Inputs:
        ...
        params    = length XX tuple (b_ellipse, upsilon, ltilde, chi_n)
...
def get_cons(r, w, b, b_splus1, n, BQ, net_tax, params):
    ...
    Inputs:
        ...
        param    = length 3 tuple (e, lambdas, g_y)
...
def get_C(cons, params):
    ...
    Inputs:
        cons        = [T,S,J] array, household consumption
        params      = length 3 tuple (pop_weights, lambdas, method)
        pop_weights = [S,] vector, population weights by age (Sx1 array)
    ...
    pop_weights, lambdas, method = params

    aggC_presum = c * pop_weights * lambdas
    if method == 'SS':
